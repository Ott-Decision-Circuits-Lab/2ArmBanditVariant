...
%         logoddsReward = log(Ppredict) - log(1 - Ppredict);
    ...
    case 'Cued' % currently only designed for 1-arm
        ...
        if ~SessionData.SettingsFile.GUI.RestartInvalidTrial % only make sense if not loop
            ...
            for iTrial = 1:SessionData.nTrials
                if ~isnan(SessionData.RawEvents.Trial{iTrial}.States.InvalidTrial(1,1))
                    iTrialInvalidCInTime = SessionData.RawEvents.Trial{iTrial}.States.InvalidTrial(:,1) - ...
                                           SessionData.RawEvents.Trial{iTrial}.States.StartCIn(1:end-1,1);
                    InvalidCInTime = [InvalidCInTime; iTrialInvalidCInTime];
                    InvalidTrialRestartIdx = [InvalidTrialRestartIdx; (1:height(iTrialInvalidCInTime))'];
                    InvalidTrialRewardProb = [InvalidTrialRewardProb; TrialRewardProb(iTrial) * ones(size(iTrialInvalidCInTime))];
            ...
            InvalidAttemptTable = table(InvalidCInTime, InvalidTrialRestartIdx,...
                                        InvalidTrialRewardProb, InvalidBF, InvalidEW,...
                                        'VariableNames', {'CInTime', 'Idx', 'Pr', 'BF', 'EW'});

            InvalidAttemptAxes = axes(FigHandle, 'Position', [0.76    0.57    0.22    0.13]);
        end
